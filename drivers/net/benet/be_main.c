...
static netdev_tx_t be_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	...
	bool flush = true /* Not in RHEL6 - !skb->xmit_more */;
	u16 wrb_cnt;
...
static void be_set_mc_list(struct be_adapter *adapter)
{
	...
	netif_addr_lock_bh(adapter->netdev);

	i = 0;

	/* cache the mc-list in adapter */
	netdev_for_each_mc_addr(mc, adapter->netdev) {
		ether_addr_copy(adapter->mc_list[i].mac, mc->dmi_addr);
		i++;
	}
	adapter->mc_count = netdev_mc_count(adapter->netdev);

	netif_addr_unlock_bh(adapter->netdev);
...
static void be_set_uc_list(struct be_adapter *adapter)
{
	...
	curr_uc_macs = netdev_uc_count(adapter->netdev);
...
static const struct attribute_group be_hwmon_group;
...
static void be_remove(struct pci_dev *pdev)
{
	...
	if (be_physfn(adapter) && adapter->hwmon_info.hwmon_dev) {
		hwmon_device_unregister(adapter->hwmon_info.hwmon_dev);
		adapter->hwmon_info.hwmon_dev = NULL;
		sysfs_remove_group(&pdev->dev.kobj, &be_hwmon_group);
	}
...
static int be_probe(struct pci_dev *pdev, const struct pci_device_id *pdev_id)
{
	...
	if (be_physfn(adapter)) {
		if (!sysfs_create_group(&pdev->dev.kobj, &be_hwmon_group)) {
