...
static int bnx2x_init_bp(struct bnx2x *bp)
{
	...
	/* Set TPA flags */
	if (bp->disable_tpa) {
		netdev_extended(bp->dev)->hw_features &= ~NETIF_F_LRO;
...
static void bnx2x_vlan_rx_add_vid(struct net_device *dev, unsigned short vid)
{
	struct bnx2x *bp = netdev_priv(dev);
	struct bnx2x_vlan_entry *vlan;
			  ...
			  struct net_device *dev, unsigned long board_type)
{
	...
	if (IS_PF(bp)) {
		if (chip_is_e1x)
			...
			hw_features |= NETIF_F_HW_VLAN_FILTER;
#ifdef CONFIG_BNX2X_SRIOV
